...
export default function AdminInvoiceModal({ isOpen, onClose, invoice, customer }) {
    ...
    const handlePrint = () => {
        // Simple trick to print only the modal content
        const printContent = document.getElementById('admin-invoice-printable');
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-sm overflow-y-auto">
            <div className="relative w-full max-w-4xl bg-[#f8fafc] rounded-xl shadow-2xl overflow-hidden mt-20 mb-20">
                {/* Header Actions */}
                <div className="sticky top-0 z-10 flex justify-between items-center p-4 bg-white border-b border-slate-200 shadow-sm">
                    <h2 className="text-lg font-bold text-slate-800">Invoice #{invoice.invoice_number}</h2>
                    <div className="flex gap-2">
                        <button
                            onClick={handlePrint}
                            className="px-4 py-2 bg-[#011D52] text-white rounded-md text-xs font-bold uppercase tracking-widest hover:bg-[#02143a] transition-colors"
                        >
                            <i className="fa-solid fa-print me-2"></i> Print Invoice
                        </button>
                        <button
                            onClick={onClose}
                            className="px-4 py-2 bg-slate-100 text-slate-600 rounded-md text-xs font-bold uppercase tracking-widest hover:bg-slate-200 transition-colors"
                        >
                            Close
                        </button>
                    </div>
            ...
            <style dangerouslySetInnerHTML={{ __html: `
                @media print {
                    ...
                    #admin-invoice-printable, #admin-invoice-printable * {
                        visibility: visible;
                    }
