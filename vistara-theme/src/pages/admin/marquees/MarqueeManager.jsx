...
const MarqueeManager = () => {
    ...
    const activeMarquee = marquees.find(m => m.is_active);
    ...
    const handleToggleActive = async (id) => {
        try {
            // First deactivate all, then activate this one (if frontend logic is strict)
            // Or just update this one if backend handles it.
            // I'll update all locally and send the specific update.
            await Promise.all(marquees.map(m => 
                marqueeService.updateMarquee(m._id, { is_active: m._id === id })
            ));
            toast.success('Broadcast node updated');
            fetchMarquees();
        } catch (error) {
                ...
                <div className="bg-[#011d52] text-white rounded-xl p-4 shadow-sm border border-slate-200 overflow-hidden group">
                    <div className="flex items-center px-4 gap-6">
                        ...
                        <marquee className="text-[12px] font-bold tracking-wide cursor-default text-white" scrollamount="6">
                            {activeMarquee.title && (
                                <span className="font-bold mr-4 uppercase tracking-wider bg-white/10 px-2 py-0.5 rounded-md">[{activeMarquee.title}]</span>
                            )}
                            <span className="mr-20">{activeMarquee.content}</span>
                        </marquee>
                    </div>
                </div>
            )}
                                        ...
                                        <td className="px-4 py-2 text-center">
                                            {m.is_active ? (
                                                ...
                                                <button 
                                                    ...
                                                    className="inline-flex items-center px-2 py-1 rounded text-[8px] font-bold bg-white text-slate-500 border border-slate-200 uppercase tracking-widest hover:bg-slate-50 transition-all">
                                                    Activate
                                                </button>
                                            )}
                                        </td>
